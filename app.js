...
const genProfile = document.querySelector('.generate');
...
genProfile.addEventListener('click', function() {
    const canvas = document.getElementById("npcBust");
    const dataURL = canvas.toDataURL("png");
    genProfile.href = dataURL;
});
...
function drawCanvas() {
    ...
    // Draw the background
    ctx.drawImage(spriteSheet, 0 + (curSel["background"] * 32), 288, 32, 32, 0, 0, 128, 128);

    // Draw the sprite
    ctx.drawImage(genColourSprite(spriteSheet, 448, 0, 32, 32, PALETTE[curColor["shirt"]], 4), 0, 16, 128, 128);
    ctx.drawImage(genColourSprite(spriteSheet, 608, 0 + (curSel["pattern"] * 32), 32, 32, PALETTE[curColor["pattern"]], 4), 0, 16, 128, 128);
    ctx.drawImage(spriteSheet, 192 + (curSel["head"] * 32), 0, 32, 32, 0, 16, 128, 128);
    ctx.drawImage(spriteSheet, 320 + (curSel["eyes"] * 32), 0, 32, 32, 0, 16, 128, 128);
    ctx.drawImage(genColourSprite(spriteSheet, 0, 0 + (curSel["hair"] * 32), 32, 32, PALETTE[curColor["hair"]], 5), 0, 0, 128, 128);
}
